...
export const ValidationPanel: React.FC<ValidationPanelProps> = ({
    ...
    return (
        <div className="absolute bottom-6 right-6 w-96 max-h-[80vh] flex flex-col gap-2 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {analysis && !hasErrors && (
                <div className="bg-slate-900/95 backdrop-blur-md border border-green-500/30 rounded-lg shadow-2xl overflow-hidden">
            ...
            {/* Errors */}
            {hasErrors && (
                <div className="bg-slate-900/95 backdrop-blur-md border border-red-500/30 rounded-lg shadow-2xl overflow-hidden">
                    ...
                    <div className="p-4">
                        {analysis && <p className="text-slate-400 text-xs mb-3 italic">{analysis}</p>}
                        <ul className="space-y-2">
                            {errors.map((error, i) => (
                                <li key={i} className="text-red-300 text-sm flex gap-2 items-start">
            ...
            {/* Warnings */}
            {hasWarnings && (
                <div className="bg-slate-900/95 backdrop-blur-md border border-yellow-500/30 rounded-lg shadow-2xl overflow-hidden">
                    ...
                    <div className="p-4">
                        <ul className="space-y-2">
                            {warnings.map((warning, i) => (
                                <li key={i} className="text-yellow-300 text-sm flex gap-2 items-start">
                                    <span className="mt-1">•</span>
                                    <span>{warning}</span>
                                </li>
                            ))}
